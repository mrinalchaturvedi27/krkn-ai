...
For this run, I kept the same fixture cluster running, installed `kube-prometheus-stack` in the `monitoring` namespace, port-forwarded Prometheus on `localhost:9090`, and queried the Prometheus HTTP API directly.
...
This is useful because it shows the suggestion logic should not invent HTTP queries when the cluster does not expose those metrics.
